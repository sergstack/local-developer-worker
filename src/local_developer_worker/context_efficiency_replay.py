...
ROOT = {"contract_version", "mode", "baseline_revision", "candidate_revision", "pairs"}


def analyze_replay(manifest: dict[str, Any]) -> dict[str, Any]:
    if set(manifest) != ROOT or manifest["contract_version"] != "1.0.0" or manifest["mode"] not in {"dry_run", "live"} or not all(isinstance(manifest[field], str) and manifest[field] for field in ("baseline_revision", "candidate_revision")) or not isinstance(manifest["pairs"], list) or not manifest["pairs"]:
    ...
    for pair in manifest["pairs"]:
        ...
        delta = {field: round((candidate[field] - base[field]) * 100 / base[field], 4) if base[field] else None for field in ("context_bytes", "tool_calls", "latency_ms")}
        deltas.append(delta)
