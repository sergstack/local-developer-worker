...
from .offload_effect_study import offload_evaluate
...
from .review import review_build, review_render
...
def _parser() -> argparse.ArgumentParser:
    ...
    for name, action in [("test", "parse"), ("git", "facts"), ("files", "inventory"), ("evidence", "build"), ("report", "summarize"), ("benchmark", "run")]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if timeout_seconds <= 0 and key != ("log", "process"):
            ...
            output = codex_run(payload, policy) if key == ("codex", "run") else ollama_advise(payload, policy) if key == ("ollama", "advise") else offload_execute(payload, policy) if key == ("offload", "execute") else offload_evaluate(payload) if key == ("offload", "evaluate") else routing_calibrate(payload, policy) if key == ("routing", "calibrate") else routing_stats(payload, policy) if key == ("routing", "stats") else routing_explain(payload, policy) if key == ("routing", "explain") else routing_value(payload, policy) if key == ("routing", "value") else log_cluster(payload, policy) if key == ("log", "cluster") else log_process(payload, policy) if key == ("log", "process") else COMMANDS[key](payload)
