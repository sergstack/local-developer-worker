...
from .telemetry import normalize_telemetry_event, valid_codex_routing_event_v2, valid_codex_run_event, valid_offload_execution_event, valid_session_record, valid_telemetry_event, valid_usefulness_mark
...
def append_event(event: dict[str, Any], root: str | Path | None = None, *, event_date: date | None = None) -> Path:
    if not valid_telemetry_event(event) and not valid_usefulness_mark(event) and not valid_codex_run_event(event) and not valid_codex_routing_event_v2(event) and not valid_offload_execution_event(event):
