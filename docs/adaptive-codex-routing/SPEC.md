...
LDW is a deterministic, read-only evidence CLI. It has no `codex run` command and does not launch a coding agent. Existing policy, repository-root, telemetry, and ToolResult boundaries are stable and must remain compatible. The source proposal is v3 in the supplied attachment; this implementation specification resolves its open contract ambiguities before code changes.
...
10. Verification has an explicit source contract. `execution` is permitted only for routine read/documentation tasks and passes on provider completion. A mutation-capable task requires `command` or `test` verification. Both use an argv array that exactly matches a policy allowlist, direct process launch, configured absolute executable allowlist, repository cwd, bounded output, and timeout. `test` status must be derived through the existing test parser with `command_observed=true` and the observed exit code.
...
16. Add `codex_run_event_v1` telemetry with an exact allowlist: `record_type`, `schema_version`, `run_id`, `profile`, `model_alias`, `effort`, `terminal_status`, `verification_status`, `fallback_count`, `escalation_count`, `input_tokens`, `cached_input_tokens`, `output_tokens`, and `reasoning_output_tokens`. Token fields are nullable non-negative integers. Credits are intentionally excluded because Codex JSONL has no stable credit field.
