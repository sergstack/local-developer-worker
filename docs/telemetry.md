...
`[telemetry].retention_days` defines a logical observation window; the default is 90 days. `ldw routing stats` reports the retained population, the current-revision population, and records excluded as incompatible or stale. `ldw routing calibrate` requires its `max_age_days` not to exceed telemetry retention. Neither command deletes, rewrites, compacts, or uploads journal partitions. Physical cleanup is a separate destructive action and requires an explicit dry-run manifest and owner approval.
